...
#nasjonalt bosted
nbo <- read_delim(
  file = "data/nasjonalt_bosted.csv", 
  delim = ";",
  ...
  mutate(
    # Fikser verdiene i variabelen naering slik at de følger
    # R-reglene for variabelnavn. Ikke strengt nødvendig, 
    # men kan gjøre livet lettere for oss senere
    naering = str_replace_all(naering, pattern = "[-.,\\s]+", "_"),
    naering = paste("k", naering, sep="_")
  )
```
...
```{r}
#nasjonalt arbeidssted
nar <- read_delim(file = "data/nasjonalt_bosted.csv", 
                 delim = ";",
                 show_col_types = FALSE
                 )
```
```{r}
#Samme eksersis som for nbo_w
```
